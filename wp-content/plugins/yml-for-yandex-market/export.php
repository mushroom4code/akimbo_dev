...
function yfym_export_page() { 
 ...
 if (isset($_REQUEST['yfym_submit_action'])) {
  if (!empty($_POST) && check_admin_referer('yfym_nonce_action', 'yfym_nonce_field')) {
	...
	yfym_optionUPD('yfym_type_prefix', sanitize_text_field($_POST['yfym_type_prefix']), $numFeed, 'yes', 'set_arr');
	...
	if (isset($_POST['yfym_params_arr'])) {
      ...
      if (isset($_POST['yfym_variations_arr'])) {
          yfym_optionUPD('yfym_variations_arr', serialize($_POST['yfym_variations_arr']), $numFeed);
      } else {yfym_optionUPD('yfym_variations_arr', serialize(array()), $numFeed);}
	 ...
	 <div class="postbox">
	   ...
	   <div class="inside">		
		<table class="form-table"><tbody>
         ...
         <tr>
             <th scope="row"><label for="yfym_type_prefix"><?php _e('Тип товара', 'yfym'); ?></label></th>
             <td class="overalldesc">
                 <select name="yfym_type_prefix" id="yfym_type_prefix">
                     <option value="off" <?php selected($typePrefix, 'disabled'); ?>><?php _e('Disabled', 'yfym'); ?></option>
                     <option value="sku" <?php selected($typePrefix, 'sku'); ?>><?php _e('Substitute from SKU', 'yfym'); ?></option>
                     <?php foreach (yfym_get_attributes() as $attribute) : ?>
                         <option value="<?php echo $attribute['id']; ?>" <?php selected($typePrefix, $attribute['id']); ?>><?php echo $attribute['name']; ?></option>
                     <?php endforeach; ?>
                 </select><br />
                 <span class="description"><?php _e('Элемент', 'yfym'); ?> <strong>TypePrefix</strong><br /></span>
             </td>
         </tr>
		...
		<?php do_action('yfym_after_manufacturer_warranty', $numFeed); ?>
         ...
         <tr>
             <th scope="row"><label for="yfym_variations_arr"><?php _e('Атрибуты вариантов товара', 'yfym'); ?></label>
             </th>
             <td class="overalldesc">
                 <select id="yfym_variations_arr" style="width: 100%;" name="yfym_variations_arr[]" size="8" multiple>
                     <?php foreach (yfym_get_attributes() as $attribute) : ?>
                         <option value="<?php echo $attribute['id']; ?>"<?php if (!empty($variations_arr)) {foreach ($variations_arr as $value) {selected($value, $attribute['id']);}} ?>><?php echo $attribute['name']; ?></option>
                     <?php endforeach; ?>
                 </select><br />
                 <span class="description"><?php _e('Элемент', 'yfym'); ?> <strong>variant</strong></span><br />
                 <span class="description" style="color: blue;"><?php _e('Hint', 'yfym'); ?>:</span> <span class="description"><?php _e('To select multiple values, hold down the (ctrl) button on Windows or (cmd) on a Mac. To deselect, press and hold (ctrl) or (cmd), click on the marked items', 'yfym'); ?></span>
             </td>
         </tr>
		</tbody></table> 
