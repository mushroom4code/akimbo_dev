...
final class Y4YM_Plugin_Upd {
	...
	public function get_info() {
		...
		switch ( $c ) {
			...
			case "402":
				...
				break;
			case "412":
				$message = sprintf( '<span style="font-weight: 700;">YML for Yandex Market %1$s:</span> %2$s! %1$s %3$s. <a href="/wp-admin/admin.php?page=yfymexport">%4$s</a> %5$s <a href="https://icopydoc.ru/product/%6$s/?utm_source=%6$s&utm_medium=organic&utm_campaign=in-plugin&utm_content=license-err&utm_term=notice" target="_blank">%7$s</a>.',
					$this->list_plugin_names[ $this->get_pref()]['name'],
				...
				break;
			case "418":
				$message = sprintf( '<span style="font-weight: 700;">YML for Yandex Market %1$s:</span> %2$s! <a href="/wp-admin/admin.php?page=yfymexport">%3$s</a> %4$s <a href="https://icopydoc.ru/product/%5$s/?utm_source=%5$s&utm_medium=organic&utm_campaign=in-plugin&utm_content=license-limit&utm_term=notice" target="_blank">%6$s</a>.',
					$this->list_plugin_names[ $this->get_pref()]['name'],
					__(
						'This license cannot be used on this site. The package limit has been exceeded',
						'yml-for-yandex-market'
	...
	public function check_update( $transient ) {
		...
		if ( ( $response_code == 200 ) && $response_message == 'OK' ) {
			...
			new YFYM_Error_Log( 'Ошибка проверки наличия обновлений. Код ошибки: ' . $response_code . '; response_message: ' . $response_message . '; Файл: class-yfym-plugin-upd.php; Строка: ' . __LINE__ );
		}
		return $transient;
	}
	...
	public function plugin_api_check_info( $result, $action, $args ) {
		if ( isset( $args->slug ) && ( $args->slug === $this->slug ) ) {
			...
			if ( ( $response_code == 200 ) && $response_message == 'OK' ) {
				...
				new YFYM_Error_Log( 'Ошибка запроса инфы о плагине. Код ошибки: ' . $response_code . '; response_message: ' . $response_message . '; Файл: class-yfym-plugin-upd.php; Строка: ' . __LINE__ );
				return $result;
			}
