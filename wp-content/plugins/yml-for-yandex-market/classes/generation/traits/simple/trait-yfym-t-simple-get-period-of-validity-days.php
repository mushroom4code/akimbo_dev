...
trait YFYM_T_Simple_Get_Period_Of_Validity_Days {
	public function get_period_of_validity_days($tag_name = 'period-of-validity-days', $result_xml = '') {
		...
		$tag_value = apply_filters('y4ym_f_simple_tag_value_period_of_validity_days', $tag_value, array('product' => $product), $this->get_feed_id());
		if (!empty($tag_value)) {	
			$tag_name = apply_filters('y4ym_f_simple_tag_name_period_of_validity_days', $tag_name, array('product' => $product), $this->get_feed_id());
			$result_xml = new YFYM_Get_Paired_Tag($tag_name, $tag_value);
		}

		$result_xml = apply_filters('y4ym_f_simple_tag_period_of_validity_days', $result_xml, array('product' => $product), $this->get_feed_id());
		return $result_xml;
	}
}
?>