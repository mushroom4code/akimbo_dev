...
trait YFYM_T_Simple_Get_Country_Of_Orgin {
	public function get_country_of_origin($tag_name = 'country_of_origin', $result_xml = '') {
		...
		$tag_value = apply_filters('y4ym_f_simple_tag_value_country_of_origin', $tag_value, array('product' => $product), $this->get_feed_id());
		if (!empty($tag_value)) {	
			$tag_name = apply_filters('y4ym_f_simple_tag_name_country_of_origin', $tag_name, array('product' => $product), $this->get_feed_id());
			$result_xml = new YFYM_Get_Paired_Tag($tag_name, $tag_value);
		}

		$result_xml = apply_filters('y4ym_f_simple_tag_country_of_origin', $result_xml, array('product' => $product), $this->get_feed_id());
		return $result_xml;
	}
}
?>