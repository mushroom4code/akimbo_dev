...
function appendProductInKapsuls( $productsData, $kapsulsData ): array {
	...
	foreach ( $productsData['cells'] as $key => $product ) {
		...
		if ( isset( $kapsulsData[ $product[0] ] ) && $kapsulsData[ $product[0] ]['name'] === $product[2] ) {
			...
			$prodWC      = new WC_Product( $productData[0]->ID );

			foreach ( $prodWC->get_gallery_image_ids() as $image ) {
				if ( wp_get_attachment_caption( $image ) === $product[1] ) {
					$img = $image;
				}
			}
...
//	name kapsul
	...
	[nectar_highlighted_text style="full_text"]<h3>' . $Data['name'] . '</h3>[/nectar_highlighted_text]
	[nectar_highlighted_text style="full_text"]' . $Data['description'] . '[/nectar_highlighted_text]
	[/vc_column]
	[/vc_row]';
...
// product list
	...
	foreach ( $Data['products_list'] as $product ) {
		...
		[image_with_animation image_url="' . $product['image'] . '" alignment="" animation="Fade In" border_radius="none" box_shadow="none" max_width="100%"]
		[nectar_food_menu_item style="default" item_name_heading_tag="h5" item_id="' . $product['ID'] . '" 
		item_name="' . $product['name'] . '" item_price="' . $product['price'] . ' ₽"]
		[/vc_column]';
		if ( ( $i % 4 ) == 0 ) {
			$item .= '[/vc_row]
