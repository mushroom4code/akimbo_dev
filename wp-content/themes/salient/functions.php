...
function orderby_first_date( $query ) {
	...
	if ( $ff !== false ) {
		$query = str_replace( 'wp_posts.menu_order,', '', $query );
	}
 ...
 * @return void
 */
function sendSettingsAjax(): void {

	if ( $_POST['action'] === 'sendSettingsAjax' && ! empty( $_POST['textInfoOtpusk'] ) ) {
		if ( ! empty( $_POST['textInfoOtpusk'] ) ) {
			update_option( 'textInfoOtpusk', $_POST['textInfoOtpusk'] );
		}
		update_option( 'сheckedInfoOtpusk', $_POST['сheckedInfoOtpusk'] );
	}
	wp_send_json( 'true', 200 );
}
...
function includeSlickSliderFiles() {
    ...
    wp_enqueue_style('catalog_product_slick_slider_style', $tpl_uri . '/css/catalog_product_slick_slider_style.css',
        array(), filemtime($tpl_uri .'/css/catalog_product_slick_slider_style.css' ));
    wp_enqueue_script('slick-slider-js', 'https://cdn.jsdelivr.net/npm/slick-carousel@1.8.1/slick/slick.min.js');
    wp_enqueue_script('catalog_product_slick_slider', $tpl_uri . '/js/catalog_product_slick_slider.js',
        array(), filemtime($tpl_uri.'/css/catalog_product_slick_slider_style.css'));
}
...
function changeProductsPrices() {
    try {
        ...
        while ($loop->have_posts()) {
            $loop->the_post();
            $product_s = new WC_Product_Variable($loop->post->ID);
            $arr_variants_prices = [];
            foreach ($product_s->get_available_variations() as $variation) {
                $variation_meta = get_post_meta($variation['variation_id']);
                $curVariantPrice = $variation_meta['_price'][0];
                if ($variation_meta['_sale_price']) {
                    $curVariantPrice = $variation_meta['_sale_price'][0];
                    if ($variation_meta['_sale_price'][0] !== $variation_meta['_price'][0]) {
                        update_post_meta($variation['variation_id'], '_price', $variation_meta['_sale_price'][0]);
                    }
                } else if ($variation_meta['_regular_price']) {
                    $curVariantPrice = $variation_meta['_regular_price'][0];
                    if ($variation_meta['_regular_price'][0] !== $variation_meta['_price'][0]) {
                        update_post_meta($variation['variation_id'], '_price', $variation_meta['_regular_price'][0]);
                    }
                }
                wc_delete_product_transients($variation['variation_id']);
                $arr_variants_prices[] = $curVariantPrice;
                if (!empty($arr_variants_prices)) {
                    $curProductPrice = $arr_variants_prices[0];
                    foreach ($arr_variants_prices as $price) {
                        $curProductPrice = min((int)$curProductPrice, (int)$price);
                    }
                }
                update_post_meta($loop->post->ID, '_price', $curProductPrice);
...
$transient = 'oneTimePriceChange';
if (is_user_logged_in()) {
    if(get_current_user_id() === get_user_by('login',  '1s-admin')->ID) {
        if (!get_transient($transient)) {
            set_transient($transient, 3600);
            add_action('init', 'changeProductsPrices');
        }
    }
}