...
function yfym_groupPrice($postId, $product, $data, $numFeed) {	// https://yandex.ru/support/partnermarket/offers.html
    ...
    $variations_arr = $data['variations_arr'];
    ...
    if ($product->is_type('variable')) {
        yfym_error_log('FEED № '.$numFeed.'; У нас вариативный товар. Файл: groupPrice.php; Строка: '.__LINE__, 0);
        ...
        for ($i = 0; $i<1; $i++) {
            ...
            */
            ...
            if ($price_yml == 0 || empty($price_yml)) {yfym_error_log('FEED № '.$numFeed.'; Вариация товара с postId = '.$postId.' пропущена т.к нет цены; Файл: groupPrice.php; Строка: '.__LINE__, 0); continue;}
            ...
            if ($yfym_skip_missing_products === 'on') {
                //enterego
                if (is_available_in_stock($product->id) == false) {yfym_error_log('FEED № '.$numFeed.'; Вариация товара с postId = '.$postId.' пропущена т.к ее нет в наличии; Файл: groupPrice.php; Строка: '.__LINE__, 0); continue;}
            ...
            if ($skip_backorders_products === 'on') {
                if ($offer->get_manage_stock() == true) { // включено управление запасом
                    if (($offer->get_stock_quantity() < 1) && ($offer->get_backorders() !== 'no')) {yfym_error_log('FEED № '.$numFeed.'; Вариация товара с postId = '.$postId.' пропущена т.к запрещен предзаказ и включено управление запасом; Файл: groupPrice.php; Строка: '.__LINE__, 0); continue;}
                } else {
                    if ($offer->get_stock_status() === 'onbackorder') { // предзаказ на уровнее вариации включен
                        yfym_error_log('FEED № '.$numFeed.'; Вариация товара с postId = '.$postId.' пропущена т.к запрещен предзаказ на уровне вариации и отключено управление запасом; Файл: groupPrice.php; Строка: '.__LINE__, 0); continue;
                    }
                }
            }
            ...
            if (($yfym_skip_products_without_pic === 'on') && ($picture_yml == '')) {
                yfym_error_log('FEED № '.$numFeed.'; Вариация товара с postId = '.$postId.' пропущена т.к нет картинки даже в галерее; Файл: groupPrice.php; Строка: '.__LINE__, 0); continue; /*continue;*/
            }

            $skip_flag = apply_filters('yfym_skip_flag_variable', $skip_flag, $postId, $product, $offer, $numFeed); /* c версии 3.2.6 */
            if ($skip_flag === true) {yfym_error_log('FEED № '.$numFeed.'; Вариативный товар с postId = '.$postId.', offer_id = '.$offer_id.' пропущен по флагу; Файл: groupPrice.php; Строка: '.__LINE__, 0); return $result_yml;}
            if ($skip_flag === 'continue') {yfym_error_log('FEED № '.$numFeed.'; Вариация товара с с postId = '.$postId.', offer_id = '.$offer_id.' пропущена по флагу; Файл: groupPrice.php; Строка: '.__LINE__, 0); $skip_flag = false; continue;}
            ...
            if (empty($no_group_id_arr)) {
                ...
                if (in_array($CurCategoryId, $no_group_id_arr)) {
                    ...
                    if ($param_at_name == '') {
                        yfym_error_log('FEED № '.$numFeed.'; Пропускаем товар т.к. нет атрибутов вариаций для уникальзации заголовков. Файл: groupPrice.php; Строка: '.__LINE__, 0);
                        yfym_error_log('FEED № '.$numFeed.'; Пропускаем товар т.к. нет атрибутов вариаций для уникальзации заголовков. Файл: groupPrice.php; Строка: '.__LINE__, 0);
...
//                if ($variation->get_variation() == false) {
//                    // это обычный атрибут
//                    $variation_val = $product->get_attribute(wc_attribute_taxonomy_name_by_id($variation->get_id()));
//                } else {
//                    $variation_val = $offer->get_attribute(wc_attribute_taxonomy_name_by_id($variation->get_id()));
//                }
//                // если этот параметр не нужно выгружать - пропускаем
...
//                $result_yml .= '<param name="'.htmlspecialchars($variation_name).'">'.ucfirst(yfym_replace_decode($variation_val)).'</param>'.PHP_EOL;
//                // $param_at_name .= ucfirst(urldecode($param_val)).' ';
//            }
//            $result_yml .= '</variant>'.PHP_EOL;
            }
...
//		$result_yml .= "<name>".htmlspecialchars($result_yml_name_itog, ENT_NOQUOTES)."</name>".PHP_EOL;
            ...
            $result_url = htmlspecialchars(get_permalink($postId));
...
//			break;
                default:
                    $result_yml .= '<model>'. $data['result_yml_name'] .'</model>'.PHP_EOL;
//				$model = (int)$model;
...
//                break;
                default:
                    $typeProduct = explode(' ', $result_yml_name);
                    $result_yml .= '<typePrefix>'.$typeProduct[0].'</typePrefix>'.PHP_EOL;
...
//                }
        ...
        yfym_error_log('FEED № '.$numFeed.'; Все вариации выгрузили. '.$ids_in_yml.' Файл: groupPrice.php; Строка: '.__LINE__, 0);

        return array($result_yml, $ids_in_yml); // все вариации выгрузили
    } // end if ($product->is_type('variable'))
    /* end Вариации */

    yfym_error_log('FEED № '.$numFeed.'; У нас обычный товар. Файл: groupPrice.php; Строка: '.__LINE__, 0);
    ...
    if ($price_yml == 0 || empty($price_yml)) {yfym_error_log('FEED № '.$numFeed.'; Товар с postId = '.$postId.' пропущен т.к нет цены; Файл: groupPrice.php; Строка: '.__LINE__, 0); return $result_yml;}
    if (class_exists('YmlforYandexMarketPro')) {
        if ((yfym_optionGET('yfymp_compare_value', $numFeed, 'set_arr') !== false) && (yfym_optionGET('yfymp_compare_value', $numFeed, 'set_arr') !== '')) {
            $yfymp_compare_value = yfym_optionGET('yfymp_compare_value', $numFeed, 'set_arr');
            $yfymp_compare = yfym_optionGET('yfymp_compare', $numFeed, 'set_arr');
    ...
    $yfym_skip_missing_products = yfym_optionGET('yfym_skip_missing_products', $numFeed, 'set_arr');
    yfym_error_log('FEED № '.$numFeed.'; $yfym_skip_missing_products = '.$yfym_skip_missing_products.'; gettype = '.gettype($yfym_skip_missing_products).'; Файл: groupPrice.php; Строка: '.__LINE__, 0);
    if ($yfym_skip_missing_products === 'on') {
        if ($product->is_in_stock() == false) {yfym_error_log('FEED № '.$numFeed.'; Товар с postId = '.$postId.' пропущен т.к нет в наличии; Файл: groupPrice.php; Строка: '.__LINE__, 0); return $result_yml;}
    }
    ...
    if ($skip_backorders_products === 'on') {
        if ($product->get_manage_stock() == true) { // включено управление запасом
            if (($product->get_stock_quantity() < 1) && ($product->get_backorders() !== 'no')) {yfym_error_log('FEED № '.$numFeed.'; Товар с postId = '.$postId.' пропущен т.к запрещен предзаказ и включено управление запасом; Файл: groupPrice.php; Строка: '.__LINE__, 0); return $result_yml; /*continue;*/}
        } else {
            if ($product->get_stock_status() !== 'instock') {yfym_error_log('FEED № '.$numFeed.'; Товар с postId = '.$postId.' пропущен т.к запрещен предзаказ; Файл: groupPrice.php; Строка: '.__LINE__, 0); return $result_yml; /*continue;*/}
        }
    }
    ...
    $yfym_skip_products_without_pic = yfym_optionGET('yfym_skip_products_without_pic', $numFeed, 'set_arr');
    if (($yfym_skip_products_without_pic === 'on') && ($picture_yml == '')) {
        yfym_error_log('FEED № '.$numFeed.'; Товар с postId = '.$postId.' пропущен т.к нет картинки даже в галерее; Файл: groupPrice.php; Строка: '.__LINE__, 0); return $result_yml; /*continue;*/
    }
